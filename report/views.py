...
def ReportListAllSales(request):
    ...
    cursor.execute ('SELECT i.sale_no as "Sale No", i.date as "Date" '
                             ...
                             '  JOIN sale_line_item s ON s.sale_no = i.sale_no '
                             ' ')
    dataReport = dict()
...
def ReportListAllSalesLineItem(request):
    ...
    cursor = connection.cursor()
    cursor.execute ('SELECT ili.sale_no as "Sale No", p.code as "Product Code"'
...
class PaymentMethodDelete(View):
    def post(self, request):
        ...
        code = request.POST['code']
        payment_method = PaymentMethod.objects.get(code=code)
        data = dict()
        if payment_method:
